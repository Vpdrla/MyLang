...
단일 C++ 파일(~4,200줄)에 백엔드 셋을 구현했습니다: 트리워킹 인터프리터, 독립 실행 파일을 만드는
...
- **완전한 언어** — 함수(재귀, 호이스팅), 클래스(생성자, 메서드, `self`), 리스트/딕셔너리(참조 방식, `==` 깊은 비교, `+` 리스트 연결, `copy()` 깊은 복사), 문자열 보간(`"이름: {x}"`), UTF-8 글자 단위 문자열, `try/catch`, `import`, 파일 입출력, 내장 함수 30개+
